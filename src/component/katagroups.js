...
export default class KataGroups extends React.Component {
  static propTypes = {
    kataGroups: React.PropTypes.instanceOf(Array).isRequired
  };
          ...
          <div className="pure-menu">
            <ul className="pure-menu-list">
              ...
              {kataGroups.map(kataGroup => <li className="pure-menu-item">
                <a href={`#kataGroup=${encodeURIComponent(kataGroup.name)}`} className="pure-menu-link">{kataGroup.name} <span
                  className="email-count">({kataGroup.katasCount})</span></a>
              </li>)}
            </ul>
          </div>
        </div>
